...
Module S := EnsEm0.RawEnsembles L. (* small sets *)
Module B := EnsEm.RawEnsembles L. (* big sets *)
...
split; intros.
 ...
 assert (#exists2 x', injU (f i) == x' & Q x').
  Tdestruct spec.
  Texists (injU x).
 ...
 assert (#exists2 x', S.eq_set (f j) x' & P x').
  Tdestruct h.
  Texists (f j).
   apply S.eq_set_refl.
...
Axiom ttrepl_ax : S.ttrepl S.eq_set.
...
assert (Rinjm : forall x0 x' y y' : S.set,
         S.in_set x0 x ->
         S.eq_set x0 x' -> S.eq_set y y' -> R (injU x0) (injU y) -> R (injU x') (injU y')).
{intros.
 revert H5; apply iff_impl; apply H; apply lift_eq; trivial. }
assert (Rinjuniq : forall x0 y y' : S.set,
           S.in_set x0 x -> R (injU x0) (injU y) -> R (injU x0) (injU y') -> S.eq_set y y').
{intros.
 apply down_eq.
 ...
 *apply U_intro.
 *apply U_intro. }
Tdestruct (S.ttrepl_implies_repl_ex
             ttrepl_ax x (fun x y => R (injU x) (injU y)) Rinjm Rinjuniq) as (b,Hb).
Texists (injU b); [apply U_intro|].
split; intros. 
...
Axiom ttcoll_ax : S.ttcoll S.eq_set.
...
assert (Rinjm : Proper (S.eq_set==>S.eq_set==>iff) (fun x y => R (injU x) (injU y))).
{do 3 red; intros.
 apply H; apply lift_eq; trivial. }
(* We use collection on small sets *)
Tdestruct (S.collection_ax ttcoll_ax a' (fun x y => R (injU x) (injU y)) Rinjm) as (B,HB).
Texists (injU B); [apply U_intro|].
intros.
Tdestruct (H1 _ H2).
...
Tdestruct H6.
Texists (injU x3); [apply lift_in; trivial|].
...
*intros.
 ...
 assert (Rinjm : forall x0 x' y y' : S.set,
         S.in_set x0 x ->
         S.eq_set x0 x' -> S.eq_set y y' ->
         (exists h:injU x0 ∈ I, F (B.eli I _ h) == injU y) ->
         exists h:injU x' ∈ I, F (B.eli I _ h) == injU y').
 {intros.
  destruct H6.
  assert (injU x' ∈ I).  
 ...
 assert (Rinjuniq : forall x0 y y' : S.set,
            S.in_set x0 x ->
            (exists h:injU x0 ∈ I, F (B.eli I _ h) == injU y) ->
            (exists h:injU x0 ∈ I, F (B.eli I _ h) == injU y') -> S.eq_set y y').
 {intros.
 ...
 Tdestruct (S.ttrepl_implies_repl_ex
              ttrepl_ax x (fun x' y => exists h:injU x' ∈ I, F (B.eli I _ h) == injU y)
              Rinjm Rinjuniq) as (B,HB).
